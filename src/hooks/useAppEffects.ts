...
export function useAppEffects() {
  ...
  useEffect(() => {
    ...
    const unlocked = evaluateAchievements(s).filter((a) => a.unlocked).map((a) => a.id)
    ...
    if (firstAchvRun.current) {
      ...
      if (unlocked.length) useAppStore.getState().markUnlocked(unlocked)
      useAppStore.getState().clearUnlockToast()
      return
    }
    if (fresh.length) {
      useAppStore.getState().markUnlocked(unlocked)
